...
import api, { getHabilitations, createHabilitation, deleteHabilitation, prolongHabilitation } from '@/lib/api';
...
export default function AdminHabilitationView() {
    const { user } = useAuth();
    ...
    if (!isAdmin) {
        return (
            <div className="max-w-7xl mx-auto px-4 py-8 text-center text-red-400 font-bold">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                <div>
                    <h1 className="text-3xl font-extrabold tracking-tight text-white">
                        Gestion des Habilitations (Admin)
                    </h1>
                    <p className="text-gray-400 mt-1">
                        Ajoutez, modifiez ou supprimez les habilitations et visites médicales des techniciens.
                    </p>
                </div>
            ...
            {showCreateModal && (
                <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex justify-center items-center p-4">
                    <div className="bg-secondary/95 border border-white/10 rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl animate-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleCreateHabilitation} className="p-6 space-y-4 font-sans">
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-primary font-bold"
                                >
                                    <option value="" className="bg-secondary text-white">Sélectionner un collaborateur...</option>
                                    {users.map(u => (
                                        <option key={u.id} value={u.id} className="bg-secondary text-white">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-primary font-bold"
                                >
                                    <option value="sst" className="bg-secondary text-white">Sauveteur Secouriste du Travail (SST)</option>
                                    <option value="caces" className="bg-secondary text-white">CACES (Engins de chantier)</option>
                                    <option value="elec" className="bg-secondary text-white">Habilitation Électrique</option>
                                    <option value="hauteur" className="bg-secondary text-white">Travail en Hauteur</option>
                                    <option value="medical" className="bg-secondary text-white">Visite Médicale Périodique</option>
                                    <option value="other" className="bg-secondary text-white">Autre Habilitation / Certificat</option>
                                </select>
                            </div>
